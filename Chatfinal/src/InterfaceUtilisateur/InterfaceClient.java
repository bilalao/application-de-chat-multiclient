...
public class InterfaceClient extends JFrame{
	...
	public void initialisation(){
		...
		jbEnvoier=new JButton("Send");




		jp2=new JPanel();
		jbConnecter=new JButton("Login");
		jtLogin=new JTextField("Enter your login");
		jtLogin.selectAll();
		...
		jtAffichage.setBackground(jtLogin.getSelectionColor());
	}
	...
	public void remplissageJPPrincipale(){
		jlEntete=new JLabel();
		ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/entete.PNG"));
		...
		jlEntete.setForeground(Color.green);
	...
	public void remplissageTable(String [][]tab){
		...
		jtabClientConnecter.setSelectionBackground(Color.BLUE);
			...
			if(e.getSource().equals(jtLogin)){
				if(e.getKeyChar()=='\n'){
					...
					if(len!=0){
						...
						if(s.length()!=len){
							image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/ErrorIcon.png"));
							JOptionPane.showMessageDialog(null,"Login ("+jtLogin.getText()+") contains spaces ","Warning...",JOptionPane.YES_NO_OPTION,image);
						}
						else
						{
							image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/edit_f2.png"));
							String adrServeur=(String) JOptionPane.showInputDialog(null,"Enter @IP of the server : ","@IP Server",1,image,null,"");
	...
	class TraitemmentButtonAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource().equals(jbConnecter)){
				...
				if (e.getActionCommand().indexOf("Login")!=-1){
					ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/edit_f2.png"));
					String adrServeur=(String) JOptionPane.showInputDialog(null,"Enter @IP of Server : ","@IP SERVER",1,image,null,"");
					...
					client.deconnecter();
					//ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/InformationIcon.png"));
					//JOptionPane.showMessageDialog(null,"You are disconnected from the server","Logout...",JOptionPane.YES_OPTION,image);
				}
			}
			else{
				if(e.getSource().equals(jbEnvoier)){
					...
					if(pos>=0){
						...
						ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("images/WarningIcon.png"));
						JOptionPane.showMessageDialog(null,"You have to choose an contact","Warning...",JOptionPane.YES_OPTION,image);
					}
				}
			}
	...
	class Client implements Runnable{
			...
			try{
				while(true){
					...
					if(typeMsgRecue.equalsIgnoreCase("ac")){
						msgRecue = "connection established at "+(((Date) obj)).toString().substring(11,20);
		...
		public void envoieMessage(){
			try{
				...
				ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("src/images/error.png"));
				JOptionPane.showMessageDialog(null,"Message sending error","Error",JOptionPane.YES_NO_OPTION,image);
				System.out.println("Serialization error "+e);
			}
		}
		public void deconnecter(){
			try{
				...
				ImageIcon image = new ImageIcon(Toolkit.getDefaultToolkit().getImage("src/images/error.png"));
				JOptionPane.showMessageDialog(null,"Disconnection error...","Erreur",JOptionPane.YES_NO_OPTION,image);
				jbConnecter.setText("Login");
				System.out.println("Serialization error "+e);
			}
		...
		public void run(){
			...
			while(true){
				try{
					...
					if(jtLogin.getText().equalsIgnoreCase("Enter your login")){
